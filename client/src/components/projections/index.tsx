...
import { Maybe, Shipper, Entry, Product, Vessel } from 'types';
...
const ShipperProjections = () => {
  ...
  const vessels = (data ? data.nodes : []) as Vessel[];
    ...
    (date) =>
      !allVessels.find(
        (vessel) =>
          isDateGreaterThanOrEqualTo(
            new Date(
              getVesselValue(vessel as Vessel, 'departureDate').value.replace(
                /-/g,
                '/',
              ),
            ),
  ...
  const allEntries = allVessels
    .map((vessel) => pathOr([], ['entries', 'nodes'], vessel))
    .flat() as Entry[];
  ...
  const products = sortBy(
    (product) =>
      ['species', 'variety', 'size', 'packType', 'plu']
        ...
        .join(' '),
    uniqBy((product) => product.id, pluck('product', allEntries) as Product[])
      .map(
        (product) =>
          ({
            ...product,
  ...
  const hasValidVesselNames = allVessels.reduce(
    (acc, vessel) =>
      ...
      vessel &&
      !['', 'New Vessel'].includes(
        getVesselValue(vessel as Vessel, 'vesselName').value,
      ),
    true,
  );
  ...
  const hasValidVesselDates = allVessels.reduce(
    (acc, vessel) =>
      ...
      vessel &&
      isAfter(
        new Date(getVesselValue(vessel as Vessel, 'arrivalDate').value),
        new Date(getVesselValue(vessel as Vessel, 'departureDate').value),
      ),
    true,
  );
  ...
  const handleSave = () => {
    ...
    if (validate()) {
      ...
      handleCreateProducts({
        ...
        const createdProducts = res.data?.bulkUpsertProduct?.products || [];
  ...
  const handleNewVessel = (newVessel: NewVessel) => {
    ...
    setChanges({
      ...
      newEntries: [...changes.newEntries, ...newEntries] as Entry[],
    });
  ...
  const handleNewProduct = (newProduct: NewProduct) => {
    ...
    setChanges({
      ...changes,
      newProducts: [...changes.newProducts, product] as Product[],
      newEntries: [...changes.newEntries, ...newEntries] as Entry[],
    });
          ...
          {!!selectedShipper ? (
            <>
              <Vessels
                ...
                vessels={allVesselsWithGhostVessels as Vessel[]}
              />
              <Products
                {...changeProps}
